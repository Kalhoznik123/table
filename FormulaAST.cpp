...
double Evaluate(const std::function<double(Position)>& func) const override {
    ...
    double result = 0;
      switch (type_) {
      case Type::Add:
          result = lhs_->Evaluate(func) + rhs_->Evaluate(func);
          break;
      case Type::Subtract:
          result = lhs_->Evaluate(func) - rhs_->Evaluate(func);
          break;
      case Type::Multiply:
          ...
          break;
      case Type::Divide:
          if(!std::isfinite(lhs_->Evaluate(func) / rhs_->Evaluate(func))){
              FormulaError error(FormulaError::Category::Div0) ;
              throw error;
          }
          result = lhs_->Evaluate(func) / rhs_->Evaluate(func);
          break;
